...
class Probase:
    ...
    def syntactic_extraction(self, filename):
        ...
        patterns1 = [r'(.*) such as (.*)',
                     r'(.*) including (.*)', r'(.*) especially (.*)']
        patterns2 = [r'(.*) and other (.*)', r'(.*) or other (.*)']

        nlp = spacy.load('en')

        with open(filename, 'r', encoding='ISO-8859-1') as corpus:
            for sentence in tqdm(corpus):
                ...
                for pattern in patterns1:
                    ...
                    if super_concepts:
                        ...
                        for sub_concept in match.group(2).split(','):
                            ...
                            sub_concept = sub_concept.translate(
                    ...
                    if super_concepts:
                        ...
                        for sub_concept in match.group(1).split(','):
                            ...
                            sub_concept = sub_concept.translate(
                                str.maketrans("", "", string.punctuation))
                            sub_concepts.append(sub_concept)
                        self.sub_concepts_corpus.append(sub_concepts[::-1])
                ...
                if super_concepts:
                    self.super_concepts_corpus.append(super_concepts)
                    sub_concepts = []
                    for sub_concept in match.group(3).split(','):
                        sub_concept = sub_concept.strip()
                        sub_concept = sub_concept.lower()
                        sub_concept = sub_concept.translate(
                            str.maketrans("", "", string.punctuation))
...
if __name__ == "__main__":
    ...
    parser.add_argument('corpus_file', help='Path to corpus')
    parser.add_argument('output_file', help='Path to ouput file')
    ...
    probase.syntactic_extraction(args.corpus_file)
    ...
    probase.save_file(args.output_file)
